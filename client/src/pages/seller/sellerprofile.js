...
const SellerProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !addresses.every(
        (addr) => addressRegex.test(addr.street) && addressRegex.test(addr.city)
      )
    ) {
          ...
          {activeTab === "profile" && user && (
            <div className="profile-info mt-4">
              ...
              <div className="profile-details-info">
                <p
                  className="flex text-lg  text-gray-800 mb-2 gap-2"
                  style={{ alignItems: "center" }}
                >
                  <strong>{t("name")}:</strong> {user.profile.name}
                  <span
                    className={`user-status py-1 px-2 rounded text-lg font-medium ${
                      user.status === "active"
                  ...
                  >
                    <p className="p-0 m-0">
                      ...
                      {user.status === "restricted" && t("Restricted")}
                      {user.status === "banned" && t("Banned")}
                    </p>
                  </span>
                </p>
