...
const Catalogue = () => {
  ...
  const productreviews = async (product) => {
    try {
      setLoading(true);
      if (!product._id) {
        alert("Product ID Empty/Invalid");
        return;
      }
      ...
      if (response.status === 200) {
        ...
        navigate("/tailor/product/reviews", {
          state: { reviews: response.data, product },
        });
      } else {
        ...
        <div className="p-3 space-y-2">
          <div className="flex justify-between items-center space-x-1">
            ...
            <div className="flex border-1 rounded space-x-2">
              <button
                onClick={() =>
                  navigate("/tailor/product/add", { state: { product } })
                }
          ...
          <span
            className="flex text-center bg-orange-500 text-white rounded mt-2 py-2 m-0 cursor-pointer"
            onClick={() => productreviews(product)}
          >
            Reviews
          </span>
        </div>
